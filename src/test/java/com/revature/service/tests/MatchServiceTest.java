...
public class MatchServiceTest {
	...
	/**
	 * Ensures the configuration is such that the match service can be autowired for
	 * this test
	 * 
	 * @author Tyler Bade
	 *
	...
	/**
	 * Mock beans are created here and the Match service is created. All dependent
	 * classes must be made into mock beans, otherwise the program will be unable to
	 * obtain the application context (at least from what I experienced)
	 */
	@InjectMocks
	@Autowired
	...
	/**
	 * These are variables that will be needed in this file Drivers will eventually
	 * hold the 4 drivers declared later The four filters will test all possible
	 * branches of the getFilteredMatches method The rider is a required variable
	 * for all methods in the match service The list of likes and dislikes is for
	 * the affect methods, and is also used in weights
	 */
	...
	/**
	 * Initializes variables for use in testing test values are given to all users,
	 * drivers are added to the list The user and map clients are mocked here, as
	 * they are used in the Match service
	 * 
	 * @throws Exception
	 */
	@Before
	...
	/**
	 * This test checks all of the methods in the MatchService class It will run as
	 * though the rider is in the database, and returns an array of drivers That
	 * driver array is pushed through the remaining operations and is compared to
	 * the original list
	 * 
	 * @throws Exception
	 */
	...
	public void getMatchesInDatabase() throws Exception {
		/**
		 * This will force the userClient mock to act as if it found matches for the
		 * rider This declaration makes sure we do not actually need the user service to
		 * test this
		 */
